...
if (!function_exists('unread_project_messages')) {
    function unread_project_messages($project_id, $client_id, $freelancer_id)
    {
        $project_conversation = ProjectConversation::where('project_id',  $project_id)
                                                    ->where('client_id',  $client_id)
                                                    ->where('freelancer_id',  $freelancer_id)
                                                    ->first();
        if ($project_conversation) {
            if (($project_conversation->client_id == Auth::user()->id) && $project_conversation->client_notify == 1) {
                return 1;
            }elseif(($project_conversation->freelancer_id == Auth::user()->id) && $project_conversation->freelancer_notify == 1){
                return 1;
            }
        }
        return 0;
...
if (!function_exists('get_payment_types')) {
    ...
    {
        $payment_types_query = PaymentType::query();
        return $payment_types_query->get();
    }
}

// Addon Activation Check
if (!function_exists('addon_is_activated')) {
    function addon_is_activated($identifier = null)
    {
        $addon = Addon::query();
        $addon = $addon->where('unique_identifier', $identifier)->where('activated', 1)->first();
