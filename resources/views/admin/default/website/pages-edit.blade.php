	...
	<form action="{{ route('custom-pages.update', $page->id) }}" method="POST" enctype="multipart/form-data">
		...
		<div class="card-body">
			<div class="form-group">
				...
				<input type="text" class="form-control" placeholder="{{ translate('Title') }}" name="title" value="{{ $page->title }}">
			</div>
			<div class="form-group ">
				<label>{{ translate('Link') }}</label>
				<div class="input-group">
					<div class="input-group-prepend"><span class="input-group-text">{{ route('home') }}/</span></div>
					<input type="text" class="form-control" placeholder="{{ translate('page-title') }}" name="slug" value="{{ $page->slug }}">
				</div>
				<small class="form-text text-muted">{{ translate('Use character, number, hypen only') }}</small>
			</div>
			<div class="form-group">
				<label>{{ translate('Add Content') }}</label>
				<textarea class="aiz-text-editor form-control" placeholder="{{ translate('Content..') }}" data-min-height="400" name="content">@php
						echo $page->content
					@endphp
				</textarea>
			</div>
		...
		<div class="card-body">
			...
			<div class="form-group">
				...
				<textarea class="resize-off form-control" placeholder="{{ translate('Keyword, Keyword') }}" name="keywords">@php
						echo $page->keywords
					@endphp
				</textarea>
