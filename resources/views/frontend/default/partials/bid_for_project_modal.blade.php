...
<form class="form-horizontal" action="{{ route('bids.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <div class="form-group">
            <input type="number" min="0" step="0.01" class="form-control form-control-sm" name="amount" placeholder="{{ translate('Enter your price') }}" required>
        </div>
    </div>
    <div class="form-group">
        <label class="form-label">
    ...
    <div class="form-group text-right">
        <button type="submit" class="btn btn-sm btn-primary transition-3d-hover mr-1 rounded-1">{{ translate('Submit') }}</button>
    </div>
</form>
