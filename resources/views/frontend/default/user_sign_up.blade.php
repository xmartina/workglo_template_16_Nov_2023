					...
					<div class="card-body">
						...
						<form class="" id="reg-form" method="POST" action="{{ route('register') }}">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" placeholder="{{ translate('Full Name') }}"  required>
                                @error('name')
							...
							<div class="form-group mb-4">
								<div class="aiz-radio-inline">
									...
									<label class="aiz-radio">
										<input type="radio" name="user_type" value="client" @if($type=='client') checked @endif> {{ translate('As A Client') }}
										<span class="aiz-rounded-check"></span>
									</label>
								</div>
								@error('user_type')
							...
							<div class="mb-5">
                                <button type="submit" class="btn btn-block btn-primary rounded-1">{{ translate('Join With Us') }}</button>
                            </div>
